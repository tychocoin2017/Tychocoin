...
#if defined(NDEBUG)
# error "Tychocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xace959a266b85fe194e3ae0d67c7ba1d13afda2a6377175b9f2d69e75f3cbd76");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tychocoin: starting difficulty is 1 / 2^12
...
{
    // Tychocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Tychocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60;; // Tychocoin: 1 Day
static const int64 nTargetSpacing = 10 * 60; // Tychocoin: 10 Minutes.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tychocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tychocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xace959a266b85fe194e3ae0d67c7ba1d13afda2a6377175b9f2d69e75f3cbd76");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Tychocoin 22 March 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1490162878;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1815425;
        ...
        assert(block.hashMerkleRoot == uint256("0xcd6708fac23d094486e2b2dbf23a8d4432d9d98e24b2ac714803e449293aabba"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Tychocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TychocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TychocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TychocoinMiner(CWallet *pwallet)
{
    printf("TychocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tychocoin-miner");
    ...
    try { loop {
        ...
        printf("Running TychocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TychocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TychocoinMiner, pwallet));
}
